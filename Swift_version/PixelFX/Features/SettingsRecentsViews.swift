        ...
        NavigationView {
            Group {
                if appModel.projectStore.projects.isEmpty {
                    ...
                    ScrollView {
                        LazyVStack(spacing: PixelFXDesign.spacingS) {
...
struct RecentProjectRow: View {
    ...
    var body: some View {
        ...
        Button(action: onTap) {
            HStack(spacing: PixelFXDesign.spacingM) {
                ProjectThumbnailView(path: project.thumbnailPath)
                    .frame(width: 72, height: 72)
                    .clipShape(RoundedRectangle(cornerRadius: PixelFXDesign.cornerSmall, style: .continuous))
...
struct ProjectThumbnailView: View {
    ...
    var body: some View {
        ...
        .task {
            ...
            PixelFXAssets.resolveImage(sourcePath: path) { loaded in
                image = loaded
            }
        }
    }
            ...
            ScrollView {
                VStack(spacing: PixelFXDesign.spacingL) {
                    section {
                        ...
                        actionRow(title: appModel.text("settings.rateApp", fallback: "Rate PixelFX")) {
                            alertItem = AlertItem(title: "Rate PixelFX", message: "This will open the App Store rating dialog")
                        }
                        actionRow(title: appModel.text("settings.contactSupport", fallback: "Contact Support")) {
                            alertItem = AlertItem(title: "Contact Support", message: "This will open email composer to support@pixelfx.app")
                        }
                        actionRow(title: appModel.text("settings.privacyPolicy", fallback: "Privacy Policy")) {
                            alertItem = AlertItem(title: appModel.text("settings.privacyPolicy", fallback: "Privacy Policy"), message: "This will open pixelfx.app/privacy")
                        }
                        actionRow(title: appModel.text("settings.termsOfService", fallback: "Terms of Service")) {
                            alertItem = AlertItem(title: appModel.text("settings.termsOfService", fallback: "Terms of Service"), message: "This will open pixelfx.app/terms")
                        }
                    }
                }
                .padding(PixelFXDesign.screenPadding)
