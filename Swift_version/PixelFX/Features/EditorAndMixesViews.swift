...
struct EditorView: View {
    ...
    var body: some View {
        ...
        NavigationView {
            VStack(spacing: 0) {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: PixelFXDesign.spacingL) {
                        ScrollView(.horizontal, showsIndicators: false) {
                            HStack(spacing: PixelFXDesign.spacingS) {
                                ForEach(appModel.effectRegistry.categories) { category in
                                    Button(appModel.categoryName(category)) {
                                        selectedCategory = category
                                        PixelFXHaptics.selection(enabled: appModel.preferences.hapticFeedback)
                                    }
                                    .buttonStyle(.plain)
                            ...
                            HStack(spacing: PixelFXDesign.spacingS) {
                                ForEach(categoryEffects) { effect in
                                    Button {
                                        ...
                                        VStack(spacing: PixelFXDesign.spacingS) {
                                        ...
                                        .padding(PixelFXDesign.spacingS)
                                        .frame(width: 126)
                        ...
                        if let currentEffect, selectedEffectID != nil {
                            VStack(alignment: .leading, spacing: PixelFXDesign.spacingS) {
                                HStack {
                                    ...
                                    .buttonStyle(.plain)
                                    .padding(.horizontal, PixelFXDesign.spacingS)
                                    .padding(.vertical, 6)
    ...
    private func topBar(theme: PixelFXTheme) -> some View {
        HStack {
            Button {
                saveCurrentProject()
                dismiss()
            ...
            HStack(spacing: 24) {
                Button {
                    ...
                    PixelFXHaptics.impact(.light, enabled: appModel.preferences.hapticFeedback)
                } label: {
    ...
    private func loadSession() async {
        ...
        PixelFXAssets.resolveImage(sourcePath: session.sourceImagePath) { loaded in
            sourceImage = loaded
    ...
    private func selectEffect(_ effect: EffectDefinition) {
        ...
        commitHistory()
        renderPreview()
        PixelFXHaptics.impact(.medium, enabled: appModel.preferences.hapticFeedback)
    }
...
struct MixesView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showPicker) {
            PhotoGridPickerView { sourcePath, size in
                ...
                PixelFXAssets.resolveImage(sourcePath: sourcePath) { loaded in
                    sourceImage = loaded
                    renderPreview()
                }
            }
    ...
    private func previewCard(theme: PixelFXTheme) -> some View {
        VStack(spacing: PixelFXDesign.spacingS) {
            if let previewImage {
                Image(uiImage: previewImage)
                    .resizable()
                    ...
                    .clipShape(RoundedRectangle(cornerRadius: PixelFXDesign.cornerXLarge, style: .continuous))
            } else {
                VStack(spacing: PixelFXDesign.spacingS) {
                ...
                .padding(PixelFXDesign.spacingXL)
            }
        ...
        .padding(PixelFXDesign.spacingM)
        .pixelFXCard(theme: theme)
    }

    private func stackSection(theme: PixelFXTheme) -> some View {
        VStack(alignment: .leading, spacing: PixelFXDesign.spacingS) {
            HStack {
            ...
            if mixStack.isEmpty {
                VStack(alignment: .leading, spacing: PixelFXDesign.spacingXS) {
    ...
    private func effectsSection(theme: PixelFXTheme, effects: [EffectDefinition]) -> some View {
        VStack(alignment: .leading, spacing: PixelFXDesign.spacingS) {
            HStack {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: PixelFXDesign.spacingS) {
    ...
    private func load() async {
        ...
        PixelFXAssets.resolveImage(sourcePath: currentSourcePath) { loaded in
            sourceImage = loaded
...
struct ExportView: View {
    ...
    private func exportButton(title: String, iconPath: String, theme: PixelFXTheme, action: @escaping () -> Void) -> some View {
